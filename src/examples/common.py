...
def transcribe(
    ...
    if backend == "gigaam":
        return _transcribe_gigaam(audio_path, language, model_name, word_timestamps, verbose)
    raise ValueError(f"unknown backend: {backend}")


# ─── GigaAM (русский ASR, onnx-asr на CPU) ──────────────────────────────────
# Локальный движок распознавания речи для диктовки. Пакет onnx-asr
# (onnxruntime на CPU, без torch/NeMo), модель gigaam-v3-e2e-rnnt - сразу
# с пунктуацией и нормализацией текста. Лицензия движка - см. файл лицензии
# GigaAM в пакете (авторы ai-sage).

_GIGAAM_DEFAULT_MODEL = "gigaam-v3-e2e-rnnt"


def _transcribe_gigaam(audio, lang, model_name, word_ts, verbose):
    import onnx_asr

    name = model_name if (model_name and str(model_name).startswith("gigaam")) else _GIGAAM_DEFAULT_MODEL
    key = ("gigaam", name)
    ...
    if model is None:
        ...
        if forced:
            providers = [p.strip() for p in forced.split(",") if p.strip()]
        if verbose:
            print(f"[whisper-skill] loading gigaam model={name} providers={providers}")
        model = onnx_asr.load_model(name, providers=providers)
        _loaded_models[key] = model

    try:
        text = model.recognize(str(audio))
    except Exception as e:
        # Авто-фолбэк на Whisper при любом сбое gigaam (если установлен).
        # Делает диктовку устойчивой: один битый файл не остаётся без ответа.
        if (
            os.environ.get("GIGAAM_FALLBACK_WHISPER", "1") != "0"
            and _is_apple_silicon()
            and _has_module("mlx_whisper")
        ):
            if verbose:
                print(f"[whisper-skill] gigaam failed ({e}); fallback → mlx-whisper")
            return _transcribe_mlx(audio, lang, "large-v3-turbo", word_ts, verbose)
        raise
...
def _transcribe_mlx(audio, lang, model_name, word_ts, verbose):
    ...
    # mlx_whisper.transcribe falls back to ffmpeg for file paths.
    # Read WAV via soundfile instead so ffmpeg is not required.
